...
int dominating_set_partitions(const igraph_t *graph,long int no_of_vertices,igraph_vector_ptr_t *res){
  igraph_vector_t d_set,d_set2,count;
 ...
 igraph_vector_init(&count,nsize);
  ...
  if(n)
    {	
     for(long int k=0;k<n;k++)
     {
      ...
      if((long int)VECTOR(bit_array)[(long int)VECTOR(edges)[k]]!=1)
       { 
        VECTOR(count)[(long int)VECTOR(bit_array)[(long int)VECTOR(edges)[k]]-2]++;
        degree++;
       }
     }
     ...
     long int max=degree/nsize,selectedpartition;
     Rprintf("max degree %d\n\n",max);

     long int size= igraph_vector_size(VECTOR(*res)[0]);
     selectedpartition=0;
     for(long int k=1;k<nsize;k++)     //size not considered
     {
         if(((long int)VECTOR(count)[k]>=max) && igraph_vector_size(VECTOR(*res)[k])<size){
         // Rprintf("Selectedpartion %d",k);  
          selectedpartition=k;
          }
     }
     
     VECTOR(bit_array)[(long int)VECTOR(d_set2)[i]]=selectedpartition+2;
      ...
      for(long int k=0;k<n;k++)
     {
      ...
      if(VECTOR(bit_array)[(long int)VECTOR(edges)[k]]==1)
      VECTOR(bit_array)[(long int)VECTOR(edges)[k]]=selectedpartition+2;
   
      igraph_vector_push_back (tmp,(long int)VECTOR(d_set2)[i]);
      igraph_vector_push_back (tmp,(long int)VECTOR(edges)[k]);
     }
     
      long int tmp_size=igraph_vector_size(VECTOR(*tmp));
      Rprintf("\n edge list %d\n",tmp_size);
    
    }//if
  
  igraph_vector_null(&count);   
